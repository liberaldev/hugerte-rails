...
module HugeRTE::Rails
  describe Configuration do
    it "has default options" do
      expect(Configuration.defaults).to eq("selector" => "textarea.hugerte", "cache_suffix" => "?v=#{VERSION}")
    end
    ...
    describe "#options_for_hugerte" do
      ...
      it "combines arrays of plugins into a single comma-separated string" do
        ...
        expect(config.options_for_hugerte["plugins"]).to eq("paste,table,fullscreen")
      end
      ...
      it "combines arrays of font_formats into a single semicolon-separated string" do
        ...
        expect(config.options_for_hugerte["font_formats"]).to eq("Andale Mono=andale mono,times;Comic Sans MS=comic sans ms,sans-serif")
      end

      it "quotes keys in hash values when required" do
        config = Configuration.new("valid_styles" => { "*" => "border,font-size", "div" => "width,height" })
        expect(config.options_for_hugerte["valid_styles"]).to eq({ "'*'" => "border,font-size", "div" => "width,height" })
      end

      it "works with integer values" do
        config = Configuration.new("width" => 123)
        expect(config.options_for_hugerte["width"]).to eq(123)
      end
      ...
      it "converts javascript function strings to Function objects" do
        ...
        expect(config.options_for_hugerte["oninit"]).to be_a(Configuration::Function)
        expect(config.options_for_hugerte["setup"]).to be_a(Configuration::Function)
      end
      ...
      it "converts javascript function strings to Function objects within nested hash" do
        ...
        expect(config.options_for_hugerte["nested"]["oninit"]).to be_a(Configuration::Function)
        expect(config.options_for_hugerte["nested"]["setup"]).to be_a(Configuration::Function)
      end
      ...
      describe "content_css option" do
        it "does not convert relative content_css values to asset paths" do
          ...
          expect(config.options_for_hugerte["content_css"]).to eq("./relative_styles.css")
        end
        ...
        it "does not convert relative content_css values to asset paths (when passed as comma-separated string)" do
          ...
          expect(config.options_for_hugerte["content_css"]).to eq("/layout.css,./custom1.css,../custom2.css")
        end
        ...
        it "converts content_css values to asset paths (when passed as comma-separated string)" do
          ...
          expect(config.options_for_hugerte["content_css"]).to eq("#{asset_path("editor1.css")},#{asset_path("editor2.css")}")
        end
